...
public class ResourceDetailsServiceImplTest {
    ...
    public void create() {
        ...
        Optional<Columns> column=columnsRepository.findById(1);
        if(resource.isPresent()){
            ResourceDetails newRD = new ResourceDetails( "value");
            boolean isSuccessful = resourceDetailsService.create(newRD,resource.get(),column.get());
    ...
    public void update() {
        ...
        Optional<Columns> column=columnsRepository.findById(1);
        if(resource.isPresent()){
            boolean isSuccessful = resourceDetailsService.update(resource.get(),column.get(),"newValue");
            Assert.assertTrue(isSuccessful);
        }else{
            System.out.println("failed to get resource");
        }
    ...
    public void get() {
        ...
        if(resource.isPresent()){
            ...
            ResourceDetails target = resourceDetailsService.get(rsrc,column.get());
